...
- Inspect the target React component, its CSS, and the relevant shared tokens in `src/styles/tokens.css` before editing.
...
- Use semantic tokens from `src/styles/tokens.css` before adding colors, shadows, spacing, or radii.
