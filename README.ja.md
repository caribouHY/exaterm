...
- ターミナル内蔵AIチャット
- ログ機能（手動/自動選択可能）
- ネットワーク機器向け色分け機能（Cisco IOS・Arista EOS・Juniper Junos・VyOS・Fujitsu Si-R・Allied Telesis AW+・Furukawa FITELnet対応）
...
外部制御を許可した保存済みプロファイルへ接続する場合は、「外部からの新規接続を許可」を有効にします。ホスト名またはIPアドレスを直接指定する場合は、さらに「任意の接続先への直接接続を許可」を有効にします。
