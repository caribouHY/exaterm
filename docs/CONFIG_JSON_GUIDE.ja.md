...
| `terminal_mode`            | string または null | このプロファイルで接続したときのターミナルモードです。指定できる値は `"general"`、`"cisco_ios"`、`"arista_eos"`、`"vyos"`、`"fujitsu_sir"`、`"allied_telesis_awplus"` です。未設定の場合は `"general"` として扱われます。 |
