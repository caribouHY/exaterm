...
| `ai.default_provider` | string | `"OpenAi"` | The provider selected by default in the AI panel. Supported values are `"OpenAi"`, `"Anthropic"`, `"Gemini"`, and `"Ollama"`. |
...
API keys for OpenAI, Anthropic, and Google Gemini are not stored in `config.json`. Keys registered from the Settings screen are stored in the operating system credential store.
...
| An AI provider does not appear | Cloud providers require API keys. For Ollama, check `ollama_enabled` and make sure the Ollama server is running. |
