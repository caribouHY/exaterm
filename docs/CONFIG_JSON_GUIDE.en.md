...
| Parameter             | Type    | Default | Description                                                                                                                                                                                                                                                                                   |
| --------------------- | ------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mcp.enabled`         | boolean | `false` | Master permission for local MCP access. Enable this together with a supported transport setting before using external MCP clients.                                                                                                                                                            |
...
The HTTP MCP transport has been removed. Existing HTTP-only host and port values in older config files are ignored and will be omitted the next time ExaTerm saves the settings. Users who previously used HTTP MCP must enable `mcp.enabled=true` and `mcp.stdio_enabled=true`, then configure their MCP client to launch `exaterm-mcp`.
